...
module.exports = function(lofcg, pullListDate) {
  ...
  const historicalAdditionalArgs = [pastPullListDate].concat(
    removeAdditionalArgs
  );
  const futureAdditionalArgs = [futurePullListDate].concat(
    removeAdditionalArgs
  );
  ...
  describe("remove series from list", function() {
    ...
    describe(
      "when removing series that isn't in pull list",
      confirmEmptyFirst(function() {
        helpers.testRemovingFromList(lofcg.pullList, testSeriesId, {
          get: historicalAdditionalArgs,
          remove: removeAdditionalArgs
