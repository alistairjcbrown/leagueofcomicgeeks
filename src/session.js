...
const createSession = function(username, password, callback) {
  ...
  request.post(
    { uri: loginUrl, form: credentials, followAllRedirects: true },
    (error, response, body) => {
      if (error) {
...
const validateSession = function(callback) {
  request.get(
    { uri: protectedUrl, followRedirect: false },
    (error, response) => {
      if (error) {
        return callback(error);
      }
      ...
      if (response && response.statusCode === 302) {
        ...
        return callback(
          new Error(`Unexpected redirection to ${response.headers.location}`)
        );
      }
...
const destroySession = function(callback) {
  request.get(
    { uri: logoutUrl, followAllRedirects: true },
    (error, response) => {
      if (error) {
        return callback(error);
      }
