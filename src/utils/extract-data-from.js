...
const seriesExtractor = function (response, options) {
  ...
  const extractSeriesData = function () {
    ...
    const cover = $el.find('.cover img').attr('data-original').split('?')[0];
    const publisher = $el.find('.publisher').text().trim();
    const count = parseInt($el.find('.details').text().trim(), 10);
...
const getVote = (piece, boundary) => parseFloat(piece.match(new RegExp(`${boundary}([^%]+)%`))[1].trim());
const getVotes = function ($el) {
  const consensus = 'consensus:';
  const potw = 'potw:';

  const toolbarPieces = $el.find('.comic-list-toolbar-text')
    .text()
    .trim()
    .replace(/·/g, '')
    .split('    ')
  ...
  return toolbarPieces.reduce(function (votes, piece) {
    if (piece.includes(consensus)) return _.extend({}, votes, { consensusVote: getVote(piece, consensus) });
    if (piece.includes(potw)) return _.extend({}, votes, { pickOfTheWeekVote: getVote(piece, potw) });
    return votes;
  }, { consensusVote: null, pickOfTheWeekVote: null });
};

const getCollectionStats = function ($, $el) {
  const collectionStats = $el.find('.comic-list-stats a')
    .map(function () {
      const $statButton = $(this);
      const actionText = $statButton.find('.text').text().toLowerCase();
...
const issueExtractor = function (response, options) {
  ...
  const extractIssueData = function () {
    const $el = $(this);
    const url = $el.find('.comic-title a').attr('href').trim();
    const [, id, variantId = null] = url.match(/comic\/(\d+)\/.+?(?:\?variant=(\d+))?$/);
    const name = $el.find('.comic-title a').text().trim();
    const cover = $el.find('.comic-cover-art img').attr('data-original').split('?')[0];
    const diamondSku = $el.find('.comic-diamond-sku').text().trim() || null;
    const { consensusVote, pickOfTheWeekVote } = getVotes($el);
    const { pulled, added } = getCollectionStats($, $el);

    const comicDetails = $el.find('.comic-details').text().split('·');
